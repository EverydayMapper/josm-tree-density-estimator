...
# Helper to project a point given distance (m) and bearing (rad)
# Simplified flat-earth approximation for small distances (Haversine inverse logic)                                                                                   
def project_point(start_lat, start_lon, dist_m, bearing_rad):
    R = 6378137.0 # Earth Radius in m
...
def run_analyzer():
    ...
    class PrecisionSampler(MouseListener, MouseMotionListener, KeyListener):
        ...
        def get_label_node(self, latlon, text):
            if self.label_node is None:
                ...
                self.label_node.put("place", "point")
        ...
        def mouseDragged(self, e):
            ...
            if self.step == "DRAW_BOX" and self.start_p and self.sample_way:
                w_dist = round_to_half(
                    self.start_p.greatCircleDistance(
                        LatLon(self.start_p.lat(), curr_p.lon())
                    )
                )
                h_dist = round_to_half(
                    self.start_p.greatCircleDistance(
                        LatLon(curr_p.lat(), self.start_p.lon())
                    )
                )
                # Visual update only (snap happens on release)
        ...
        def mouseReleased(self, e):
            ...
            def process_release():
                ...
                if self.step == "DRAW_BOX":
                    # 1. Calculate dimensions based on where user released
                    # We calculate bearings to ensure we project correctly
                    start_lat = self.sample_nodes[0].coor.lat()
                    start_lon = self.sample_nodes[0].coor.lon()
                    # Distances                                                      
                    w_raw = self.sample_nodes[0].coor.greatCircleDistance(
                        LatLon(start_lat, end_p.lon())
                    )
                    h_raw = self.sample_nodes[0].coor.greatCircleDistance(
                        LatLon(end_p.lat(), start_lon)
                    )
                    # Rounding                              
                    final_w = round_to_half(w_raw)
                    final_h = round_to_half(h_raw)
                    
                    
                    # 2. FORCE SNAP: Re-calculate corner coordinates based on rounded meters
                    # Bearing logic: East/West for Width, North/South for Height
                    # Simple approx: if curr_lon > start_lon, bearing is 90 deg (East), else 270 (West)
                    self.sample_area_sqm = final_w * final_h
                    self.log_box_dims = (final_w, final_h)
                    
                    # 3. Snap visuals (for UI polish)                                 
                    b_w = (
                        math.radians(90)
                        if end_p.lon() > start_lon
                        else math.radians(270)
                    )
                    b_h = (
                        math.radians(180)
                        if end_p.lat() < start_lat
                        else math.radians(0)
                    )
                    
                    # Project Node 2 (Top Right) -> Start + Width
                    l2, o2 = project_point(start_lat, start_lon, final_w, b_w)
                    self.sample_nodes[1].setCoor(LatLon(l2, o2))
                    # Project Node 4 (Bottom Left) -> Start + Height                                      
                    l4, o4 = project_point(start_lat, start_lon, final_h, b_h)
                    self.sample_nodes[3].setCoor(LatLon(l4, o4))
                    
                    # Project Node 3 (Bottom Right) -> Start + Width + Height
                    # We project from Node 4 (Bottom Left) using Width bearing                    
                    l3, o3 = project_point(l4, o4, final_w, b_w)
                    self.sample_nodes[2].setCoor(LatLon(l3, o3))
                    
                    # Create Poly for hit testing
                    poly = Path2D.Double()
                    ...
                    if dist > 0.05:
                        ...
                        self.log_calibration_data.append(
                            (p1.lat(), p1.lon(), end_p.lat(), end_p.lon(), dist)
                        )
                        l1 = Node(p1)
                        l2 = Node(end_p)
                        line = Way()
                        ...
                        self.update_status(
                            "Avg: {:.1f}m (n={}) | ENTER to start counting.".format(
                                sum(self.diameters) / len(self.diameters),
                                len(self.diameters),
                            )
                        )
                        layer.invalidate()
        ...
        def mouseClicked(self, e):
            if self.step == "COUNTING" and e.isShiftDown():
                ...
                if self.sample_poly.contains(ll.lat(), ll.lon()):
                    node = Node(ll)
                    node.put("name", str(len(self.tree_nodes) + 1))
                    node.put("natural", marker_natural)
                    layer.data.addPrimitive(node)
                    self.tree_nodes.append(node)
                    self.update_status(
                        "Count: {} | ENTER to finish".format(len(self.tree_nodes))
                    )
                    layer.invalidate()
        ...
        def keyPressed(self, e):
            if e.getKeyCode() == 10:
                if self.step == "CALIBRATE" and self.diameters:
                    ...
                    JOptionPane.showMessageDialog(
                        None,
                        "Now SHIFT+CLICK every {} inside the box.".format(singular),
                    )
                ...
                if self.step == "COUNTING" and self.tree_nodes:
    ...
    def monitor():
            ...
            try:
                ...
                if count > 0:
                    ...
                    canopy_pc = min(
                        100,
                        int(
                            round(((est_total * indiv_area) / total_area) * 100 / 5.0)
                            * 5
                        ),
                    )
                    density_class = (
                    ...
                    final_tags = {
                        ...
                        "est:avg_{}".format(tag_suffix): "{:.1f}m".format(
                            tool.avg_diameter
                        ),
                    ...
                    if curr_nat:
                        surveyed_type_str = "natural=" + curr_nat
                    elif curr_land:
                        surveyed_type_str = "landuse=" + curr_land

                    if density_class in ["dense", "very_dense"] and curr_nat == "scrub":
                        msg = "Density is {}% ({}).\nSuggest changing natural=scrub to natural=wood?".format(
                            canopy_pc, density_class
                        )
                        ...
                        density_class in ["scattered", "open"]
                        and curr_nat == "wood"
                        and curr_land != "forest"
                    ):
                        msg = "Density is {}% ({}).\nSuggest changing natural=wood to natural=scrub?".format(
                            canopy_pc, density_class
                        )
                        res = JOptionPane.showConfirmDialog(
                            None,
                    ...
                    if perform_save:
                        ...
                        for i, n in enumerate(tool.tree_nodes):
                            log += "  #{}: {:.6f}, {:.6f}\n".format(
                                i + 1, n.coor.lat(), n.coor.lon()
                            )
                        
                        # --- RESTORED COMPLETE SUMMARY MESSAGE ---
                        ...
                        if (
                            ...
                            fc.setSelectedFile(
                                java.io.File(
                                    "TreeSurvey_{}_{}.txt".format(
                                        target_id, timestamp_str
                                    )
                                )
                            )
                            if fc.showSaveDialog(None) == JFileChooser.APPROVE_OPTION:
                                with open(
                                    fc.getSelectedFile().getAbsolutePath(), "w"
                                ) as f:
                                    f.write(log)
        ...
        import java.io.File # Import needed for JFileChooser default file
